...
const DocumentCard = ({ label, filename }: { label: string, filename: string }) => (
...
const AdminDashboard: React.FC = () => {
   ...
   const [selectedResume, setSelectedResume] = useState<typeof MOCK_AGENT_RESUMES[0] | null>(null);
   ...
   const handleResumeStatusChange = (status: string) => {
      ...
      const updatedResumes = agentResumes.map(resume => {
         if (resume.id === selectedResume.id) {
            let statusColor = '';
            if (status === 'SELECTED') statusColor = 'bg-emerald-50 text-emerald-600 border-emerald-100';
            if (status === 'REJECTED') statusColor = 'bg-slate-100 text-slate-500 border-slate-200';
            if (status === 'PROCESSING') statusColor = 'bg-purple-50 text-purple-600 border-purple-100'; // For HOLD

            return { ...resume, status, statusColor };
         }
         return resume;
      });
