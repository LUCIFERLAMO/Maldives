...
const MyApplicationsPage = () => {
    ...
    const renderAppItem = (app) => {
        const isAction = app.status === ApplicationStatus.ACTION_REQUIRED || app.status === 'ACTION_REQUIRED';
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide hidden md:inline-block shadow-sm ${isAction ? 'bg-amber-100 text-amber-700' :
                            ...
                            ['HOLD', 'REVIEWING'].includes(app.status) ? 'bg-blue-50 text-blue-600' :
                            ['APPROVED', 'SELECTED', 'ACCEPTED'].includes(app.status) ? 'bg-emerald-100 text-emerald-700' :
                                    'bg-slate-100 text-slate-600'
                            }`}>
                            {getDisplayStatus(app.status).label}
                        </span>
            ...
            <div className="container mx-auto max-w-4xl">
                ...
                <div className="mb-10 bg-gradient-to-r from-teal-700 via-teal-600 to-cyan-600 rounded-3xl p-8 shadow-xl shadow-teal-900/10 border border-teal-600/20 relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                        <div>
                            <h1 className="text-3xl font-bold text-white tracking-tight drop-shadow-sm">Career History</h1>
                            <p className="text-teal-100 font-medium mt-1">Track and manage your professional journey.</p>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex justify-end">
                    ...
                    <div className="relative w-full max-w-lg bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8">
                            ...
                            <div className={`rounded-2xl p-6 border ${isFinalStatus(selectedApp.status) ? (selectedApp.status === 'REJECTED' ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100') : (selectedApp.status === ApplicationStatus.ACTION_REQUIRED || selectedApp.status === 'ACTION_REQUIRED') ? 'bg-amber-50 border-amber-100 text-amber-900' : 'bg-slate-50 border-slate-100 text-slate-800'}`}>
                                ...
                                <div className={`text-xl font-bold flex items-center gap-2 ${getDisplayStatus(selectedApp.status).color.replace('bg-', 'text-').split(' ')[0]}`}>
                                    {(selectedApp.status === ApplicationStatus.ACTION_REQUIRED || selectedApp.status === 'ACTION_REQUIRED') && <AlertTriangle className="w-6 h-6 text-amber-600" />}
                            ...
                            {(selectedApp.status === ApplicationStatus.ACTION_REQUIRED || selectedApp.status === 'ACTION_REQUIRED') && (
