...
const CandidateLoginPage = ({ initialMode = 'login' }) => {
    ...
    // Handle the credential returned by Google's Identity Services popup
                        ...
                        <div className="max-w-sm mx-auto w-full animate-in fade-in slide-in-from-right-8 duration-700 relative z-10">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    if (!clientId) {
                                        setNotification({ type: 'error', text: 'Google Sign-In is not configured yet. Please use email and password.' });
                                        return;
                                    }
                                    if (window.google?.accounts?.id) {
                                        window.google.accounts.id.prompt();
                                    } else {
                                        setNotification({ type: 'error', text: 'Google Sign-In is not available. Please try again.' });
                                    }
                                }}
                                className="w-full flex items-center justify-center gap-3 border-2 border-slate-200 bg-white text-slate-700 py-4 rounded-2xl font-semibold text-sm hover:bg-slate-50 hover:border-slate-300 hover:shadow-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {googleLoading ? (
